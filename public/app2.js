...
function renderChecklistList() {
    ...
    list.innerHTML = checklists.map(cl => {
        const itemsHtml = (cl.items || []).map((item, idx) => `
            <div class="checklist-row-wrap" data-cl-id="${cl.id}" data-idx="${idx}">
                <div class="checklist-row-bg">삭제</div>
...
function initSwipeDelete() {
    $$('.checklist-row-wrap').forEach(wrap => {
        const row = wrap.querySelector('.checklist-row');
        ...
        document.addEventListener('mousemove', (e) => {
            ...
            row.style.opacity = Math.max(0, 1 - Math.abs(currentX) / 250);
        });
        ...
        document.addEventListener('mouseup', () => {
            ...
            if (Math.abs(currentX) > 120) {
                ...
                row.style.transform = `translateX(${currentX > 0 ? 500 : -500}px)`;
                row.style.opacity = '0';
                wrap.style.transition = 'max-height 0.3s, padding 0.3s, margin 0.3s';
                ...
                setTimeout(() => {
                    const clId = wrap.dataset.clId;
                    const idx = parseInt(wrap.dataset.idx);
                    deleteChecklistItem(clId, idx);
                }, 550);
            } else {
                // 원래 위치로
                row.style.transition = 'transform 0.3s, opacity 0.3s';
        ...
        row.addEventListener('touchend', () => {
            ...
            if (Math.abs(currentX) > 100) {
                ...
                row.style.transform = `translateX(${currentX > 0 ? 500 : -500}px)`;
                row.style.opacity = '0';
                wrap.style.transition = 'max-height 0.3s, padding 0.3s';
                wrap.style.maxHeight = wrap.offsetHeight + 'px';
                setTimeout(() => {
                    wrap.style.maxHeight = '0';
                    wrap.style.overflow = 'hidden';
